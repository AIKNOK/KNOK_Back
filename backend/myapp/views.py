...
def generate_resume_questions(request):
    ...
    # ✅ Claude 프롬프트 생성 
    prompt = f"""
    당신은 뛰어난 AI 면접관입니다. 아래 이력서를 기반으로 직무 관련 면접 질문 3개를 생성하세요.
    
    [이력서 내용]
    {text}
    
    [질문 작성 규칙]
    - 질문은 이력서에 실제로 등장한 기술, 프로젝트, 경험을 기반으로 작성해야 합니다.
    - 질문은 반드시 이력서 내 단어를 포함해야 합니다.
    - 모든 질문은 대문자로 시작하고, 완전한 자연어 문장이어야 하며 줄바꿈으로 구분합니다.
    - '귀하'라는 표현은 절대로 사용하지 말고, 항상 '본인'으로 대체하세요.
    - 질문 앞에는 어떤 숫자, Q1, *, - 등의 기호도 절대로 붙이지 마세요.
    - 출력은 질문 3개 문장만 포함되어야 하며, 다른 설명은 없어야 합니다.
    
    [주의사항]
    - 출력 형식은 자동 시스템에 의해 검증되며, 줄 수가 다르거나 규칙을 위반하면 질문은 폐기됩니다.
    - 출력이 정확해야 저장 및 배포됩니다.
    
    [예시 출력 형식]
    React 프로젝트에서 성능 최적화를 위해 어떤 방법을 사용하셨나요?  
    본인이 수행한 OCR 프로젝트에서 가장 도전적이었던 부분은 무엇이었나요?  
    협업 중 발생한 기술적 충돌을 본인은 어떻게 해결하셨나요?
    """
    ...
    verify_prompt = f"""
    당신은 뛰어난 AI 면접 관리자입니다. 아래 이력서를 기반으로 Claude가 생성한 질문들을 검토한 뒤, 최종적으로 사용할 수 있는 질문 3개만 출력하세요.
    ...
    [검토 목적]
    - 질문은 반드시 이력서에 실제로 언급된 기술, 경험, 프로젝트를 기반으로 작성되어야 합니다.
    - 관련 없는 질문은 삭제하거나, **이력서 내용을 반영하도록 수정**해야 합니다.
    - 질문은 설정된 난이도({difficulty}) 기준에 적합해야 하며, 특히 ‘어려움’ 난이도일 경우에는 전문성을 요구하는 형태로 보완해야 합니다.
    
    [출력 규칙 — 모든 항목 위반 시 질문은 폐기됩니다]
    - 질문은 **반드시 3개**만 출력해야 합니다. 하나라도 초과하거나 부족하면 시스템이 실패로 간주합니다.
    - 각 질문은 **이력서에 등장한 실제 단어**를 포함해야 하며, 무관한 키워드는 허용되지 않습니다.
    - 질문 앞에 숫자, Q1, -, *, 등 **어떠한 접두어나 불필요한 기호도 절대 붙이지 마세요.**
    - 질문은 **항상 대문자로 시작하는 완전한 자연어 문장**이어야 하며, 줄바꿈으로 구분해야 합니다.
    - ‘귀하’라는 표현은 절대 금지이며, 항상 ‘본인’을 사용해야 합니다.
    - 출력은 오직 질문 3줄만 포함되어야 하며, 다른 설명, 제목, 안내 문구는 절대 포함하지 마세요.
    
    [예시 형식 - 절대 그대로 쓰지 마세요]
    React 프로젝트에서 성능 최적화를 위해 어떤 방법을 사용하셨나요?  
    본인이 개발한 추천 시스템에서 사용한 핵심 알고리즘은 무엇이었나요?  
    협업 과정에서 본인이 주도한 기술적 해결 경험에 대해 말씀해 주세요.
